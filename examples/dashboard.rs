...
impl Default for Metrics {
    fn default() -> Self {
        Self {
            ...
            processes: vec![
                Process { name: "revue".into(), cpu: 12.5, memory: 45.2 },
                Process { name: "cargo".into(), cpu: 8.3, memory: 128.0 },
                Process { name: "rust-analyzer".into(), cpu: 15.2, memory: 256.4 },
                Process { name: "terminal".into(), cpu: 1.2, memory: 32.0 },
                Process { name: "nvim".into(), cpu: 3.4, memory: 64.8 },
            ],
...
impl Dashboard {
    ...
    fn render_processes(&self, ctx: &mut RenderContext) {
        ...
        Border::rounded().title("Processes").child(process_list).render(ctx);
    }

    fn render_network(&self, ctx: &mut RenderContext) {
        let in_spark = Text::new(self.render_sparkline_text(&self.metrics.network_in)).fg(Color::GREEN);
        let out_spark = Text::new(self.render_sparkline_text(&self.metrics.network_out)).fg(Color::RED);

        let net_in = Border::rounded()
            .title("Network In")
            .child(vstack()
                .child(in_spark)
                .child(Text::new("125.4 KB/s")));

        let net_out = Border::rounded()
            .title("Network Out")
            .child(vstack()
                .child(out_spark)
                .child(Text::new("42.1 KB/s")));
...
impl View for Dashboard {
    fn render(&self, ctx: &mut RenderContext) {
        ...
        let header = hstack()
            ...
            .child(Text::new(format!("  Uptime: {}", self.format_uptime())).fg(Color::rgb(128, 128, 128)));
        header.render(ctx);
