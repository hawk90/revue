...
pub struct IncrementalHandlers<T: 'static, R: 'static> {
    ...
    pub on_insert: Box<dyn Fn(&mut R, usize, T) + Send + Sync>,
    /// Handle update operation
    pub on_update: Box<dyn Fn(&mut R, usize, T, T) + Send + Sync>,
    /// Handle remove operation
    pub on_remove: Box<dyn Fn(&mut R, usize, T) + Send + Sync>,
    /// Handle replace operation (full recomputation)
    pub on_replace: Box<dyn Fn(&SignalVec<T>) -> R + Send + Sync>,
}
...
impl<T: 'static, R: 'static> IncrementalHandlers<T, R> {
    ...
    pub fn new() -> Self {
        Self {
            on_insert: Box::new(|_, _, _| {}),
            on_update: Box::new(|_, _, _, _| {}),
            on_remove: Box::new(|_, _, _| {}),
            on_replace: Box::new(|_| panic!("Replace handler not implemented")),
        }
    }

    /// Set handler for insert operations
    pub fn insert(mut self, f: impl Fn(&mut R, usize, T) + Send + Sync + 'static) -> Self {
        self.on_insert = Box::new(f);
        self
    }
...
impl<T: Clone + Send + Sync + 'static, R: Clone + Send + Sync + 'static> IncrementalComputed<T, R> {
    ...
    pub fn new(
        ...
        _handlers: IncrementalHandlers<T, R>,
    ) -> Self {
        let id = SignalId::new();
        ...
        let _cached_clone = cached.clone();
        let _init_clone = Arc::new(init);

        // Note: In a full implementation, we'd need SignalVec to emit proper diffs
        // For now, this is a simplified version that sets up the structure
