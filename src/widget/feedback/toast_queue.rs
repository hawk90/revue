...
mod tests {
    ...
    fn test_toast_queue_builder_chain() {
        ...
        assert_eq!(queue.position, ToastPosition::TopLeft);
        assert_eq!(queue.stack_direction, StackDirection::Up);
        assert_eq!(queue.max_visible, 3);
        assert_eq!(queue.default_duration, Duration::from_secs(3));
        assert_eq!(queue.gap, 2);
        assert_eq!(queue.toast_width, 50);
        assert!(!queue.deduplicate);
    }

    // =========================================================================
    // ToastEntry builder method tests
    // =========================================================================
    ...
    fn test_toast_queue_priority_ordering_multiple() {
        ...
        assert_eq!(queue.queue[0].message, "Critical");
        assert_eq!(queue.queue[1].message, "High");
        assert_eq!(queue.queue[2].message, "Normal");
        assert_eq!(queue.queue[3].message, "Low");
    }
}
